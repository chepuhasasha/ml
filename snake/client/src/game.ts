...
export class SnakeGame {
  ...
  public step(action: Action, just_check: boolean = false) {
    ...
    // Left the field?
    if (head.y === -1 || head.y > this.height - 1 || head.x === -1 || head.x > this.width - 1) {
      reward = Reward.DIE
      done = true
      return getResult()
    }
    ...
    return result
      .map((row, i) => {
        ...
        this.food
          .filter((f) => f.y === i)
          .forEach((f) => {
            updatedRow = updatedRow.substr(0, f.x) + '.' + updatedRow.substr(f.x + 1)
          })
        this.snake
          .filter((s) => s.y === i)
          .forEach((s) => {
            updatedRow = updatedRow.substr(0, s.x) + '▊' + updatedRow.substr(s.x + 1)
          })
        return updatedRow
      })
